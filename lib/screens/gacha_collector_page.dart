...
class _GachaCollectorPageState extends State<GachaCollectorPage>
  ...
  static const List<_WaifuCard> _pool = [
    // SSR (3% chance)
    _WaifuCard('Zero Two', '02', _Rarity.ssr, '💗', Color(0xFFE91E63), 'Darling in the FRANXX'),
    _WaifuCard('Rem', 'レム', _Rarity.ssr, '💙', Color(0xFF2196F3), 'Re:Zero'),
    _WaifuCard('Miku', '初音', _Rarity.ssr, '🩵', Color(0xFF00BCD4), 'Vocaloid'),
    // SR (12% chance)
    _WaifuCard('Makima', '鎖', _Rarity.sr, '🔴', Color(0xFFD32F2F), 'Chainsaw Man'),
    _WaifuCard('Yor Forger', '殺', _Rarity.sr, '🖤', Color(0xFF424242), 'Spy x Family'),
    _WaifuCard('Nezuko', '禰', _Rarity.sr, '🌸', Color(0xFFFF9800), 'Demon Slayer'),
    _WaifuCard('Mai Sakurajima', '桜', _Rarity.sr, '💜', Color(0xFF9C27B0), 'Bunny Girl Senpai'),
    _WaifuCard('Power', '力', _Rarity.sr, '🧡', Color(0xFFFF5722), 'Chainsaw Man'),
    // R (35% chance)
    _WaifuCard('Hinata', '日', _Rarity.r, '🤍', Color(0xFF7986CB), 'Naruto'),
    _WaifuCard('Mikasa', '巨', _Rarity.r, '⚔️', Color(0xFF455A64), 'Attack on Titan'),
  ...
  Future<void> _pull() async {
    if (_pulling) return;
    setState(() { _pulling = true; _lastPull = null; });
        ...
        flexibleSpace: Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [
              Colors.amber.withValues(alpha: 0.4), Colors.black.withValues(alpha: 0.95),
            ]),
          ),
        ),
      ),
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            gradient: const LinearGradient(
                              colors: [Colors.amber, Colors.deepPurple, Colors.pinkAccent]),
                            boxShadow: [BoxShadow(
              ...
              child: AnimatedBuilder(
                ...
                builder: (_, __) => Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: LinearGradient(colors: [
                      Colors.amber.shade700, Colors.deepPurple]),
                    boxShadow: [BoxShadow(
                      color: Colors.amber.withValues(alpha: 0.2 + _glowCtrl.value * 0.3),
                      blurRadius: 20, spreadRadius: -4)],
                  ),
                  child: const Center(child: Text('✨ PULL ✨',
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: card.rarity.borderColor, width: 3),
          boxShadow: [BoxShadow(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(card.kanji, style: TextStyle(
              fontSize: 48, fontWeight: FontWeight.w100, color: Colors.white.withValues(alpha: 0.3))),
            const SizedBox(height: 8),
            Text(card.name, style: const TextStyle(
              color: Colors.white, fontWeight: FontWeight.w800, fontSize: 18)),
            Text(card.anime, style: TextStyle(
            ...
            Container(
              ...
              child: Text(card.rarity.label,
                style: TextStyle(color: card.rarity.borderColor,
  ...
  Widget _buildMiniCard(_WaifuCard card) {
    return Container(
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          colors: [card.color.withValues(alpha: 0.3), card.color.withValues(alpha: 0.1)]),
        border: Border.all(color: card.rarity.borderColor.withValues(alpha: 0.5), width: 1),
      ),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          ...
          Text(card.name, style: const TextStyle(color: Colors.white, fontSize: 9,
              fontWeight: FontWeight.w700), textAlign: TextAlign.center),
          Text(card.rarity.label, style: TextStyle(
            color: card.rarity.borderColor, fontSize: 8, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
...
class _WaifuCard {
  ...
  final Color color;
  final String anime;
  const _WaifuCard(this.name, this.kanji, this.rarity, this.emoji, this.color, this.anime);
}
