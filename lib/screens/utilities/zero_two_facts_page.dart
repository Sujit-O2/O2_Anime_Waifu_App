...
class _ZeroTwoFactsPageState extends State<ZeroTwoFactsPage>
  ...
  final Map<int, bool> _unlockedFacts = {};
  ...
  void initState() {
    ...
    _bgScale = Tween<double>(begin: 1.0, end: 1.05)
        .animate(CurvedAnimation(parent: _bgCtrl, curve: Curves.easeInOut));
    _bgOpacity = Tween<double>(begin: 0.04, end: 0.08)
        .animate(CurvedAnimation(parent: _bgCtrl, curve: Curves.easeInOut));
  ...
  Future<void> _loadAffection() async {
    try {
      // AffectionService stores affection data - use a safe default
      setState(() => _affectionLevel = 250); // Default affection level
      _initializeUnlocks();
    } catch (_) {}
  }

  void _initializeUnlocks() {
    _unlockedFacts[0] = true;
    _unlockedFacts[1] = _affectionLevel >= 100;
    _unlockedFacts[2] = _affectionLevel >= 250;
    _unlockedFacts[3] = _affectionLevel >= 500;
    _unlockedFacts[4] = _affectionLevel >= 1000;
  }
  ...
  bool _isFactLocked(int index) {
    return _unlockedFacts[index] == false;
  }
  ...
  void _toggleFavorite() {
    ...
    setState(() {
      if (_favorites.contains(_currentIdx)) {
        _favorites.remove(_currentIdx);
      } else {
        _favorites.add(_currentIdx);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isLocked = _isFactLocked(_currentIdx);
              ...
              Expanded(
                child: Stack(children: [
                  ...
                  _loading
                    ...
                    : _facts.isEmpty
                      ? Center(
                        child: Text('Could not load facts.', style: GoogleFonts.outfit(color: Colors.white54)),
                      )
                      : SlideTransition(
                        position: _slideAnim,
                        child: Padding(
                            ...
                            children: [
                              if (isLocked)
                                Container(
                                  ...
                                  child: Column(children: [
                                    const Icon(Icons.lock_rounded, color: Colors.red, size: 48),
                                    ...
                                    Text('Reach ${(_currentIdx + 1) * 250} affection to unlock',
                                      textAlign: TextAlign.center,
                                      style: GoogleFonts.outfit(
                                    ...
                                    ClipRRect(
                                      ...
                                      child: LinearProgressIndicator(
                                        value: _affectionLevel / ((_currentIdx + 1) * 250),
                                    ...
                                    const SizedBox(height: 8),
                                    Text('$_affectionLevel / ${(_currentIdx + 1) * 250}',
                                      style: GoogleFonts.outfit(
                                        color: Colors.white54,
                                        fontSize: 11,
                                      ),
                              ...
                              if (_favorites.isNotEmpty) ...[
                                const SizedBox(height: 16),
                                Text(
                                  '${_favorites.length} fact${_favorites.length > 1 ? "s" : ""} favorited ✦',
