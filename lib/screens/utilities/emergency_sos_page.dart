...
class _EmergencySosPageState extends State<EmergencySosPage>
  ...
  Future<void> _load() async {
    ...
    try {
      _contacts = (jsonDecode(p.getString('sos_contacts_data') ?? '[]') as List)
          .cast<Map<String, dynamic>>();
    } catch (_) {}
    try {
      _medInfo = Map<String, String>.from(
          jsonDecode(p.getString('sos_medical_info') ?? '{}'));
    } catch (_) {}
  ...
  void _addContact() {
    ...
    setState(() => _contacts
        .add({'name': _nameCtrl.text.trim(), 'phone': _phoneCtrl.text.trim()}));
    _nameCtrl.clear();
    _phoneCtrl.clear();
    _save();
  }
  ...
  void _triggerSos() {
    if (_contacts.isEmpty) {
      _snack('❌ Add emergency contacts first!', Colors.redAccent);
      return;
    }
  ...
  void _cancelSos() {
    ...
    _snack('SOS cancelled', Colors.white38);
  }
  ...
  Future<void> _sendSos() async {
    try {
      ...
      final locationStr = pos != null
          ? 'https://www.google.com/maps?q=${pos.latitude},${pos.longitude}'
          : 'Location unavailable';
      final blood = _medInfo['blood'] ?? '';
      final allergies = _medInfo['allergies'] ?? '';
      final conditions = _medInfo['conditions'] ?? '';

      final message =
          '🆘 EMERGENCY SOS from O2-WAIFU!\n\nI need help! My location:\n$locationStr\n\n${blood.isNotEmpty ? 'Blood: $blood\n' : ''}${allergies.isNotEmpty ? 'Allergies: $allergies\n' : ''}${conditions.isNotEmpty ? 'Conditions: $conditions\n' : ''}';
              ...
              actions: [
                ...
                ElevatedButton(
                    onPressed: () {
                      setState(() => _medInfo = {
                            'blood': bCtrl.text,
                            'allergies': aCtrl.text,
                            'conditions': cCtrl.text
                          });
                      _save();
  ...
  Widget build(BuildContext context) {
    return FeaturePageV2(
      title: 'EMERGENCY SOS',
      subtitle: '${_contacts.length} emergency contacts',
      ...
      content: SingleChildScrollView(
        ...
        child: Column(children: [
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: _sendingSos
                            ...
                            : Colors.redAccent.withValues(
                                alpha: 0.1 + _pulseCtrl.value * 0.05),
                        border: Border.all(
          ...
          // Contact list
          ..._contacts.asMap().entries.map((e) => GlassCard(
                ...
                child: Row(children: [
                  Container(
                      ...
                      child: Center(
                          child: Text(
                              e.value['name'].toString()[0].toUpperCase(),
                              style: GoogleFonts.outfit(
                                  color: Colors.redAccent,
                                  fontWeight: FontWeight.w800)))),
