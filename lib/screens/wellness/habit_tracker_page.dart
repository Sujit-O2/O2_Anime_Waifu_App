...
class _HabitTrackerPageState extends State<HabitTrackerPage>
  ...
  int get _completedToday => _habits
      .where((h) => (h['completedOn'] as List).contains(_todayKey))
      .length;

  int get _bestStreak => _habits.fold<int>(
      0,
      (best, habit) => (habit['longStreak'] as int? ?? 0) > best
          ? habit['longStreak'] as int? ?? 0
          : best);

  String get _commentaryMood {
    if (_completedToday == _habits.length && _habits.isNotEmpty) {
      return 'achievement';
    }
  ...
  Future<void> _loadHabits() async {
    ...
    if (_uid != null) {
      try {
        ...
        if (kDebugMode) debugPrint('HabitTracker load Firestore error: $e');
      }
    }
    ...
    try {
      ...
      if (kDebugMode) debugPrint('HabitTracker load local error: $e');
    }
    if (mounted) {
      setState(() => _loading = false);
      _fadeCtrl.forward();
  ...
  Future<void> _saveHabits() async {
    ...
    if (_uid != null) {
      try {
        ...
        if (kDebugMode) debugPrint('HabitTracker save Firestore error: $e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return FeaturePageV2(
      title: 'HABIT TRACKER',
      subtitle: '$_completedToday/${_habits.length} done today',
                        ...
                        children: [
                          ...
                          Text(
                            _habits.isEmpty
                                ...
                                : '$_completedToday habits checked off today',
                            style: GoogleFonts.outfit(
                              color: Colors.white,
                              fontSize: 20,
                          ...
                          Text(
                            _habits.isEmpty
                                ? 'Add a habit below and start turning repeated actions into streaks.'
                                : 'Your best stored streak is $_bestStreak days. Small daily wins keep the board moving.',
                            style: GoogleFonts.outfit(
          ...
          Padding(
            ...
            child: ClipRRect(
              ...
              child: LinearProgressIndicator(
                value: _habits.isEmpty ? 0 : _completedToday / _habits.length,
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(Icons.add_rounded,
                    color: V2Theme.primaryColor, size: 22),
              ),
            ),
          ]),
        ),
          ...
          child: _loading
              ? const Center(
                  child: CircularProgressIndicator(
                      strokeWidth: 2, color: V2Theme.primaryColor))
              : _habits.isEmpty
                  ? Center(
                      child: Column(
