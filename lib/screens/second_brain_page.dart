...
class _SecondBrainPageState extends State<SecondBrainPage> {
  ...
  Future<void> _load() async {
    ...
    if (data != null) setState(() => _notes = (jsonDecode(data) as List).cast<Map<String, dynamic>>());
  }
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    if (q.isNotEmpty) list = list.where((n) => (n['title'] as String).toLowerCase().contains(q) || (n['content'] as String).toLowerCase().contains(q)).toList();
    return list;
  }
      ...
      body: Column(children: [
        ...
        Expanded(child: items.isEmpty
          ...
          : ListView.builder(padding: const EdgeInsets.all(12), itemCount: items.length, itemBuilder: (_, i) {
            ...
            return Dismissible(key: Key(n['time']), onDismissed: (_) { setState(() => _notes.remove(n)); _save(); }, background: Container(color: Colors.redAccent.withValues(alpha: 0.2), alignment: Alignment.centerRight, padding: const EdgeInsets.only(right: 20), child: const Icon(Icons.delete, color: Colors.redAccent)),
              child: Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: c.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(14), border: Border.all(color: c.withValues(alpha: 0.2))),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [Text('${_tagEmojis[n['tag']]} ', style: const TextStyle(fontSize: 16)), Expanded(child: Text(n['title'], style: GoogleFonts.outfit(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700))), Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: c.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(6)), child: Text(n['tag'], style: GoogleFonts.outfit(color: c, fontSize: 9, fontWeight: FontWeight.w700)))]),
                  if ((n['content'] as String).isNotEmpty) ...[const SizedBox(height: 6), Text(n['content'], maxLines: 3, overflow: TextOverflow.ellipsis, style: GoogleFonts.outfit(color: Colors.white54, fontSize: 12))],
                ])));
          })),
      ]),
    );
