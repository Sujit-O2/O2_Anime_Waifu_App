...
class HydrationNutritionService {
  ...
  static final HydrationNutritionService instance = HydrationNutritionService._();
  ...
  Future<void> initialize() async {
    await _loadData();
    if (kDebugMode) debugPrint('[HydrationNutrition] Initialized with $_totalMealsLogged meals logged');
  }
  ...
  Future<void> logMeal({
    ...
    if (kDebugMode) debugPrint('[HydrationNutrition] Meal logged: $description ($calories cal)');
  }
  ...
  Future<void> logWaterIntake(int amountMl) async {
    ...
    if (kDebugMode) debugPrint('[HydrationNutrition] Water logged: $amountMl ml');
  }
  ...
  String getNutritionInsights() {
    ...
    final todayLogs = _nutritionLogs.where((log) => 
    ...
    final totalCalories = todayLogs.fold<int>(0, (sum, log) => sum + log.calories);
    final totalProtein = todayLogs.fold<double>(0, (sum, log) => sum + log.protein);
    ...
    if (totalProtein < 50) {
      buffer.writeln('💡 Consider increasing protein intake for muscle maintenance');
    }
  ...
  String getHydrationInsights() {
    ...
    final today = DateTime.now();
    final todayLogs = _hydrationLogs.where((log) => 
        log.timestamp.year == today.year && 
        log.timestamp.month == today.month && 
        log.timestamp.day == today.day
    ).toList();
  ...
  String getDailyRecommendations() {
    ...
    final today = DateTime.now();
    final todayWater = _hydrationLogs.where((log) => 
        log.timestamp.year == today.year && 
        log.timestamp.month == today.month && 
        log.timestamp.day == today.day
    ...
    if (todayWater < 1500) {
      ...
      recommendations.add('Have another glass of water to reach your daily goal');
    }
    
    // Nutrition recommendation
    final todayMeals = _nutritionLogs.where((log) => 
        log.timestamp.year == today.year && 
        log.timestamp.month == today.month && 
        log.timestamp.day == today.day
    ).toList();
    ...
    if (todayMeals.isEmpty) {
      ...
      final totalCalories = todayMeals.fold<int>(0, (sum, log) => sum + log.calories);
      if (totalCalories < 800 && today.hour > 12) {  // If it's afternoon and low calories
        recommendations.add('Consider a nutritious lunch to fuel your afternoon');
      }
    }
  ...
  Future<void> _saveData() async {
    try {
      ...
      final data = {
        'nutritionLogs': _nutritionLogs.take(50).map((l) => l.toJson()).toList(),
        'hydrationLogs': _hydrationLogs.take(50).map((l) => l.toJson()).toList(),
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (jsonString != null && jsonString.isNotEmpty) {
        ...
        _nutritionLogs.addAll(
          (data['nutritionLogs'] as List<dynamic>)
              .map((l) => NutritionLog.fromJson(l as Map<String, dynamic>))
        );
        
        _hydrationLogs.clear();
        _hydrationLogs.addAll(
          (data['hydrationLogs'] as List<dynamic>)
