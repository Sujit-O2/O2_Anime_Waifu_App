...
class StressDetectionService {
  ...
  Future<void> initialize() async {
    await _loadData();
    if (kDebugMode)
      debugPrint('[StressDetection] Initialized with $_totalReadings readings');
  }
  ...
  double analyzeVoiceStress({
    ...
    final pitchStress =
        (pitchVariance - 0.5).abs() * 2; // Higher variance = more stress
    final rateStress =
        (speakingRate - 0.5).abs() * 2; // Deviations from normal rate = stress
    final volumeStress = volumeVariance; // Higher variance = more stress
    final pauseStress =
        pauseFrequency.clamp(0.0, 1.0); // More pauses = more stress

    // Weighted average
    return (pitchStress * 0.3 +
        rateStress * 0.2 +
        volumeStress * 0.3 +
        pauseStress * 0.2);
  }
  ...
  double analyzeTypingStress({
    ...
    // Normalize inputs
    final speedStress =
        (typingSpeed - 0.5).abs() * 2; // Too fast or slow = stress
    final errorStress = errorRate.clamp(0.0, 1.0); // More errors = more stress
    final backspaceStress =
        backspaceFrequency.clamp(0.0, 1.0); // More corrections = stress
    final pressureStress = pressureVariance; // Harder typing = stress

    // Weighted average
    return (speedStress * 0.25 +
        errorStress * 0.25 +
        backspaceStress * 0.25 +
        pressureStress * 0.25);
  }
  ...
  Future<void> recordStressReading({
    ...
    if (kDebugMode) {
      debugPrint(
          '[StressDetection] Stress reading recorded: ${(combinedStress * 100).toStringAsFixed(0)}%');
    }
  }
  ...
  String getStressInsights() {
    ...
    final avgStress =
        recentReadings.fold<double>(0, (sum, r) => sum + r.combinedStress) /
            recentReadings.length;

    final buffer = StringBuffer();
    buffer.writeln('😣 Stress Insights (Last 10 readings):');
    buffer.writeln(
        '• Average Stress Level: ${(avgStress * 100).toStringAsFixed(0)}%');
    ...
    if (avgStress > 0.7) {
      ...
      buffer.writeln(
          '⚠️ Moderate stress levels. Monitor your stress triggers and practice self-care.');
    } else {
      buffer.writeln(
          '🌟 Low stress levels. Your stress management appears effective.');
    }
  ...
  String getCopingStrategies() {
    if (_stressHistory.isEmpty)
      return 'Monitor your stress first to get personalized coping strategies.';

    final recentStress = _stressHistory
            .take(3)
            .fold<double>(0, (sum, r) => sum + r.combinedStress) /
        3;
  ...
  List<StressReading> getReadings({int limit = 10}) {
    return List.unmodifiable(_stressHistory.take(limit));
  }

  StressReading? get latestReading =>
      _stressHistory.isEmpty ? null : _stressHistory.first;
  ...
  Future<void> _saveData() async {
    try {
      ...
      final data = {
        'stressHistory':
            _stressHistory.take(50).map((s) => s.toJson()).toList(),
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (jsonString != null && jsonString.isNotEmpty) {
        ...
        _stressHistory.addAll((data['stressHistory'] as List<dynamic>)
            .map((s) => StressReading.fromJson(s as Map<String, dynamic>)));

        _totalReadings = data['totalReadings'] as int;
        final loadedScores =
            Map<String, double>.from(data['dailyStressScores'] ?? {});
