  ...
  void _recalcAttention() {
    ...
    for (final sig in _history) {
      ...
      if (sig.replySpeedMs != null) {
        ...
        if (ms < 5000) {
          score += 100;
        } else if (ms < 30000)  score += 75;
        else if (ms < 120000) score += 50;
        else if (ms < 600000) score += 25;
        else                  score += 0;
        signalCount++;
      }
    ...
    } else if (avg >= 40) _level = AttentionLevel.medium;
    else                _level = AttentionLevel.low;
  }
