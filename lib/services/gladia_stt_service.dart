...
class GladiaSttService {
  ...
  Future<bool> startListening() async {
    ...
    try {
      ...
      _wsSub = _ws!.stream.listen(
        _handleWsMessage,
        onError: (error) {
          print('[GladiaSTT] WebSocket error: $error');
          if (onError != null) onError!('gladia_ws_error: $error');
        },
        onDone: () {
          print('[GladiaSTT] WebSocket closed');
          if (listening) {
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        if (url != null && url.isNotEmpty) {
          print('[GladiaSTT] Session created, WebSocket URL obtained');
          return url;
        }
      }

      print('[GladiaSTT] Session init failed: ${response.statusCode} $body');
      return null;
    } on TimeoutException {
      print('[GladiaSTT] Session init timed out');
      return null;
    } catch (e) {
      print('[GladiaSTT] Session init error: $e');
      return null;
    }
  }
  ...
  void _handleWsMessage(dynamic message) {
    try {
      ...
      if (type == 'transcript') {
        ...
        print('[GladiaSTT] Server error: $errorMsg');
        if (onError != null) onError!('gladia_server_error: $errorMsg');
      }
    } catch (e) {
      print('[GladiaSTT] Parse error: $e');
    }
  }
  ...
  Future<void> stopListening() async {
    ...
    try {
      ...
      print('[GladiaSTT] Recorder stop error: $e');
    }
